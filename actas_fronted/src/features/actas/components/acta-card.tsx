...
import { CalendarDays, ChevronDown, ChevronRight, ClipboardList, Download, Eye, FileCheck2, FileStack, Loader2, MapPin, PlusCircle, QrCode, Upload } from 'lucide-react';
...
import { EvidenciasActaModal } from './evidencias-acta-modal';
    ...
    <div className="overflow-hidden rounded-xl border border-border/70 bg-card shadow-card transition-all duration-200 hover:border-accent/50 hover:shadow-soft">
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
        <Link
          ...
          title="Ver el acta virtual completa"
          className="flex items-center gap-1.5 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 text-xs font-semibold text-primary transition-colors hover:bg-primary hover:text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
        ...
        <button
          type="button"
          title="Ver código QR de asistencia"
          onClick={(event) => {
            event.stopPropagation();
            setMostrarQr(true);
          }}
          className="flex items-center gap-1.5 rounded-lg border border-accent/30 bg-accent/10 px-2.5 py-2 text-xs font-semibold text-accent-foreground transition-colors hover:bg-accent/20"
        >
        ...
        <button
          type="button"
          title="Descargar el acta en Word"
          disabled={descargando}
          onClick={(event) => {
            event.stopPropagation();
            descargarWord();
          }}
          className="flex items-center gap-1.5 rounded-lg border border-primary/20 bg-primary/5 px-2.5 py-2 text-xs font-semibold text-primary transition-colors hover:bg-primary/10 disabled:pointer-events-none disabled:opacity-50"
        >
          {descargando ? <Loader2 className="size-3.5 animate-spin" /> : <Download className="size-3.5" />}
          <span className="hidden sm:inline">{descargando ? 'Preparando...' : 'Word'}</span>
        </button>

        <button
          type="button"
          title="Ver o subir evidencias de la reunión"
          onClick={(event) => {
            event.stopPropagation();
            setMostrarEvidencias(true);
          }}
          className="flex items-center gap-1.5 rounded-lg border border-warning/40 bg-warning/10 px-2.5 py-2 text-xs font-semibold text-warning-foreground transition-colors hover:bg-warning/20"
        >
        ...
        {acta.urlActaFisica && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            title="Ver el acta física escaneada (documento oficial firmado)"
            className="flex items-center gap-1.5 rounded-lg border border-success/30 bg-success/5 px-2.5 py-2 text-xs font-semibold text-success transition-colors hover:bg-success/10"
          >
        ...
        {puedeSubirActaFisica && (
          <>
            ...
            <button
              type="button"
              title={acta.urlActaFisica ? 'Reemplazar el escaneo del acta física firmada' : 'Subir el escaneo del acta física firmada'}
              disabled={subirActaFisica.isPending}
              onClick={(event) => {
                event.stopPropagation();
                fileInputRef.current?.click();
              }}
              className="flex items-center gap-1.5 rounded-lg border border-input bg-background px-2.5 py-2 text-xs font-semibold text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground disabled:pointer-events-none disabled:opacity-50"
            >
              <Upload className="size-3.5" />
              <span className="hidden sm:inline">{acta.urlActaFisica ? 'Reemplazar' : 'Subir física'}</span>
            </button>
          </>
        )}
      ...
      {expandida && (
        <div id={`acuerdos-${acta.id}`} className="border-t border-border bg-secondary/30 px-5 py-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 rounded-lg border border-primary/20 bg-primary/5 px-2.5 py-1.5 text-xs font-semibold text-primary transition-colors hover:bg-primary hover:text-primary-foreground"
            >
          ...
          {!cargandoAcuerdos && !errorAcuerdos && !acuerdos?.length && (
            <p className="rounded-lg border border-dashed border-border bg-card/70 p-4 text-center text-sm text-muted-foreground">Esta acta todavía no tiene acuerdos registrados.</p>
          )}
          {!!acuerdos?.length && (
      ...
      {mostrarEvidencias && <EvidenciasActaModal actaId={acta.id} onClose={() => setMostrarEvidencias(false)} />}
